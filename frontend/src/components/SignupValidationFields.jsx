...
export const SignupValidationFields = ({
  ...
  const checkAvailability = useCallback(async (field, value, setStatus) => {
    if (!value) {
    ...
    try {
      ...
      const response = await fetch(`${backendUrl}/api/auth/check-availability?field=${field}&value=${encodeURIComponent(value)}`);
