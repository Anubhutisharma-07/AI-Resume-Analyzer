...
export type DataRetentionAction = 'ENCRYPT_AT_REST' | 'ANONYMIZE_PII' | 'AUTO_PURGE_EXPIRED' | 'REDACT_SENSITIVE';
...
export interface SecurityAuditTimelineLog {
  logId: string;
  timestamp: string;
  eventType: 'PII_REDACTION_APPLIED' | 'COMPLIANCE_SCAN_COMPLETED' | 'RETENTION_POLICY_TRIGGERED' | 'ENCRYPTION_KEY_ROTATED';
  details: string;
  actor: string;
