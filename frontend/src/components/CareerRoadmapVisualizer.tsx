...
import { CareerTrack } from './CareerRoadmapModel';

export default function CareerRoadmapVisualizer() {
  const [service] = useState(() => new CareerRoadmapService());
  const [currentRole, setCurrentRole] = useState<string>('Frontend Engineer');
  const [targetRole, setTargetRole] = useState<string>('Staff Frontend Architect');
  const [track, setTrack] = useState<CareerTrack>('SOFTWARE_ENGINEERING');
      ...
      <header className="mb-8 border-b border-slate-800 pb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-slate-400 text-xs font-mono">v2.8.0 • Automated Multi-Year Roadmap</span>
          </div>
          ...
          <p className="text-slate-400 text-sm mt-1 max-w-3xl">
            Generate step-by-step career progression milestones, required technical competencies, and project impact goals to achieve your target title.
          </p>
        </div>
      </header>
        ...
        {trajectory.milestones.map((ms, idx) => (
          ...
          >
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
              <div>
                <span className="text-xs font-mono text-indigo-400 font-bold">STAGE {idx + 1} ({ms.targetTimeframeMonths} Months)</span>
                <h3 className="text-xl font-extrabold text-slate-100">{ms.stageTitle}</h3>
              </div>
              <span className={`px-3 py-1 text-xs font-bold rounded-full font-mono ${ms.isCurrentStage ? 'bg-indigo-500/20 text-indigo-300 border border-indigo-500/40' : 'bg-slate-800 text-slate-400'}`}>
                {ms.isCurrentStage ? 'CURRENT STAGE' : 'FUTURE TARGET'}
              </span>
            </div>

            <p className="text-xs text-slate-300 font-mono mb-4">
              <strong className="text-indigo-300">Suggested Project Impact: </strong>{ms.suggestedProjectImpact}
            </p>

            <div className="flex flex-wrap gap-2">
              {ms.requiredSkills.map(skill => (
                <span key={skill} className="px-2.5 py-1 bg-slate-950 border border-slate-800 text-slate-300 text-[11px] rounded-md font-mono">
                  {skill}
                </span>
              ))}
            </div>
