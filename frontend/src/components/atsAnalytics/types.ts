...
export interface AtsCategoryScore {
  categoryName: 'KEYWORD_MATCH' | 'FORMATTING_PARSING' | 'QUANTIFIED_IMPACT' | 'SECTION_COMPLETION' | 'BREVITY' | 'SECURITY_COMPLIANCE';
  score: number;
...
export interface AtsAnalyticsAuditLog {
  logId: string;
  timestamp: string;
  eventType: 'SCORE_RECALCULATED' | 'KEYWORD_MATRIX_UPDATED' | 'REPORT_EXPORTED' | 'PARSER_RULE_UPDATED';
  details: string;
  performer: string;
  impactScope: string;
}
